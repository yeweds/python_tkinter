...
explain_label = tkinter.Label(window, text='Please enter an integer below,\nand then click the button "Confirm"',
...
def get_number():
    ...
    if INPUT_STRING.isdigit():
        if CONVERTER_MODE == 16:
            output_string.set(SUCCESSFUL_OUTPUT_16_1 + INPUT_STRING + SUCCESSFUL_OUTPUT_16_2 + hex(int(INPUT_STRING)))
            history_listbox.insert(0, [INPUT_STRING, hex(int(INPUT_STRING))])
        elif CONVERTER_MODE == 8:
            output_string.set(SUCCESSFUL_OUTPUT_8_1 + INPUT_STRING + SUCCESSFUL_OUTPUT_8_2 + oct(int(INPUT_STRING)))
            history_listbox.insert(0, [INPUT_STRING, oct(int(INPUT_STRING))])
        else:
            output_string.set(SUCCESSFUL_OUTPUT_2_1 + INPUT_STRING + SUCCESSFUL_OUTPUT_2_2 + bin(int(INPUT_STRING)))
...
result_label = tkinter.Label(window, textvariable=output_string, font=('Arial', 12), height=1)
result_label.place(x=175, y=105, anchor='center')
...
history_listbox.place(x=175, y=280, anchor='center')
